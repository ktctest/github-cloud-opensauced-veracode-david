...
module.factory('UserRoleExpiration', function($resource) {
    return $resource(authUrl + '/realms/:realm/user-role-expirations/api/users/:userId/user-role-expirations', {
        realm : '@realm',
        userId : '@userId'
    });
});
...
module.factory('AllClientRoleMapping', function($resource) {
    return $resource(authUrl + '/admin/realms/:realm/users/:userId/role-mappings/clients/:client', {
        realm : '@realm',
        userId : '@userId',
        client : "@client"
...
function clientSelectControl($scope, realm, Client) {
    $scope.clientsUiSelect = {
        ...
        formatResult: function(object, container, query) {
            object.text = object.name != null ? object.name : object.clientId;
            return object.name != null ? object.name : object.clientId;
        }
    };
}
...
module.factory('ClientRoleDefaultExpirationList', function($resource) {
    return $resource(authUrl + '/realms/:realm/role-expiration-durations/clients/:clientId/role-expiration-durations', {
        realm : '@realm',
        clientId : '@clientId'
    });
});

module.factory('ClientRoleDefaultExpiration', function($resource) {
    return $resource(authUrl + '/realms/:realm/role-expiration-durations/roles/:roleId/role-expiration-duration', {
        realm : '@realm',
        roleId : '@roleId'
    });
});
