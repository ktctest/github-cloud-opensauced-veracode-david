...
module.controller('ClientRoleListCtrl', function ($scope, $route, realm, client, ClientRoleList, RoleById, Notifications, Dialog, ClientRoleDefaultExpirationList, RoleClientComposites) {
    $scope.realm = realm;
    $scope.roles = [];
    $scope.client = client;
    $scope.defaultExpirations = [];
    $scope.defaultRole = realm.defaultRole;
    ...
    $scope.searchQuery = function () {
        ...
        $scope.roles = ClientRoleList.query($scope.query, function (roles) {
            $scope.searchLoaded = true;
            $scope.lastSearch = $scope.query.search;

            ClientRoleDefaultExpirationList.query($scope.query, function (res) {
                $scope.defaultExpirations = res;
                roles.forEach(r => r.expirationDurationDays = $scope.getDefaultExpirationForRole(res, r.id));
            })

            RoleClientComposites.query({
                realm : realm.realm,
                role : $scope.defaultRole.id,
                client : client.id,
            }, function(res) {
                roles.forEach(r => r.defaultRole = isDefaultRole(res, r));
            });
        });
    };

    function isDefaultRole(defaults, role) {
        let isDefault = false;
        defaults.forEach(r => {
            if (r.id === role.id) {
                isDefault = true;
    ...
    $scope.getDefaultExpirationForRole = function (exp, roleId) {
        let days = null;
        exp.forEach(e => {
            if (e.roleId === roleId) {
                days = e.expirationDurationDays;
            }
        });
        return days;
    }
...
module.controller('ClientRoleDetailCtrl', function ($scope, $route, realm, client, role, roles, Client,
                                                    ...
                                                    $http, $location, Dialog, Notifications, ComponentUtils, ClientRoleDefaultExpiration) {
    $scope.realm = realm;
    $scope.client = client;
    $scope.role = angular.copy(role);
    $scope.create = !role.name;
    $scope.defaultExpiration = undefined;
    $scope.defaultRole = realm.defaultRole;
    $scope.isDefaultRole = false;
    $scope.isDefaultRoleInitial = false;

    $scope.changed = $scope.create;

    function loadDefaultExpiration() {
        if ($scope.create) {
            return;
        }

        ClientRoleDefaultExpiration.get({
            realm: realm.realm,
            roleId: role.id,
        }, function (res) {
            $scope.defaultExpiration = res.expirationDurationDays;
        });
    }
    ...
    function loadDefaultRoles() {
        ...
        RoleClientComposites.query({
            ...
            $scope.isDefaultRole = isDefaultRole(res, role)
            $scope.isDefaultRoleInitial = isDefaultRole(res, role)
        });
    }
    ...
    function updateUserRoleExpiration() {
        $http.put(authUrl + '/realms/' + realm.realm + '/role-expiration-durations/roles/' + role.id + '/role-expiration-duration',
            JSON.stringify({roleId: role.id, expirationDurationDays: $scope.defaultExpiration})
        ).then();
    }

    function updateDefaultRole() {
        if ($scope.isDefaultRole === $scope.isDefaultRoleInitial) {
           return;
        }

        if ($scope.isDefaultRole) {
            $http.post(authUrl + '/admin/realms/' + realm.realm + '/roles-by-id/' + $scope.defaultRole.id + '/composites',
                '[' + JSON.stringify(role) + ']'
            ).then();
        } else {
            $http.delete(authUrl + '/admin/realms/' + realm.realm + '/roles-by-id/' + $scope.defaultRole.id + '/composites',
                {data: '[' + JSON.stringify(role) + ']', headers: {"Content-Type": "application/json"}}
            ).then();
        }
    }
