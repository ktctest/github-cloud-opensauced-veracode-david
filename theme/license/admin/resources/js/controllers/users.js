module.controller('UserRoleMappingCtrl', function ($scope, $http, $route, realm, user, client, Client, Notifications, RealmRoleMapping,
                                                   UserRoleExpiration, ClientRoleMapping, AvailableRealmRoleMapping, AllClientRoleMapping, AvailableClientRoleMapping,
    ...
    $scope.userRoleExpirations = UserRoleExpiration.query({realm: realm.realm, userId: user.id})

    $scope.updateUserRoleExpiration = function (expiration) {
        $http.post(authUrl + '/realms/' + realm.realm + '/user-role-expirations/api/users/' + user.id + '/user-role-expirations',
            JSON.stringify(expiration)).then(function () {
            Notifications.success($translate.instant('user.roles.expiration.add.success'));
        })
    }

    $scope.extractExpirationDate = function (clientId, roleId) {
        let client = $scope.userRoleExpirations.find(c => c.clientId === clientId);
        if (client == null) {
            return undefined;
        }

        let role = client.roles.find(r => r.roleId === roleId);
        if (role == null) {
            return undefined;
        }

        return role.expirationDate;
    }

    $scope.extractResellerID = function (clientId, roleId) {
        let client = $scope.userRoleExpirations.find(c => c.clientId === clientId);
        if (client == null) {
            return undefined;
        }
        ...
        return role.resellerID;
    }

    $scope.updateAdditionalAttributes = function (clientId) {
        let client = $scope.userRoleExpirations.find(c => c.clientId === clientId);
        if (client == null) {
            return undefined;
        }
        ...
        roles.forEach(r => {
            let expValue = document.getElementById(r.roleId + "_expiration").value;
            let resellerValue = document.getElementById(r.roleId + "_reseller").value;

            if ($scope.clientMappings.some(a => a.id === r.roleId) || $scope.clientComposite.some(a => a.id === r.roleId)) {
                if (r.expirationDate !== expValue) {
                    r.expirationDate = expValue;
                }

                if (r.resellerID !== resellerValue) {
                    r.resellerID = resellerValue;
                }

                $scope.updateUserRoleExpiration(r);
            }
        });
    }
    ...
    $scope.inArray = function (arr, elem) {
        return arr != null && elem != null && arr.some(a => a.id === elem.id);
    };

    $scope.changeClientRole = function (r) {
        if ($scope.inArray($scope.clientMappings, r)) {
            $scope.selectedClientMappings = JSON.stringify(r);
            $scope.deleteClientRole();
        } else {
            $scope.selectedClientRoles = JSON.stringify(r);
            $scope.addClientRole();
        }
    }
    ...
    $scope.changeClient = function (client) {
        ...
        if ($scope.selectedClient) {
            ...
            $scope.allClientRoles = AllClientRoleMapping.query({
                realm: realm.realm,
                userId: user.id,
                client: $scope.selectedClient.id
            });
            ...
            $scope.allClientRoles = $scope.clientRoles.concat($scope.clientMappings)
        } else {
            $scope.clientRoles = null;
            $scope.clientMappings = null;
            $scope.clientComposite = null;
            $scope.allClientRoles = null;
        }
        $scope.selectedClientRoles = [];
        $scope.selectedClientMappings = [];
    };
...
module.controller('UserListCtrl', function ($scope, realm, User, UserSearchState, UserImpersonation, BruteForce, Notifications, $route, Dialog, $translate) {
    ...
    $scope.init = function () {
        ...
        // Die erweiterte Repräsentation wird benötigt, damit man auf die Attribute des Benutzers zugreifen kann.
        $scope.query.briefRepresentation = 'false';

        if (!UserSearchState.isFirstSearch) $scope.searchQuery();
    };

    // Sucht in der Attributs-Liste des Benutzers nach einem Attribut mit dem Key "name".
    // Falls dieses Attribut einen Wert besitzt, wird der Wert zurückgegeben.
    // Ansonsten wird der Benutzername zurückgegeben.
    $scope.getDisplayName = function(u) {
        if (u.attributes == null || u.attributes.name == null || u.attributes.name.length <= 0) {
            return u.username;
        }
        return u.attributes.name[0];
    }
...
module.controller('UserTabCtrl', function ($scope, $location, Dialog, Notifications, Current) {
    ...
    $scope.getDisplayName = function() {
        if ($scope.user.attributes == null || $scope.user.attributes.name == null || $scope.user.attributes.name.length <= 0) {
            return $scope.user.username;
        }

        const nameAttr = $scope.user.attributes.name;

        if (typeof nameAttr === "object") {
            return $scope.user.attributes.name[0];
        } else {
            return $scope.user.attributes.name;
        }
    }
});
